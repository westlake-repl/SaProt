...
##### 🔴 Note: SaProt (35M and 650M) requires structural &#40;SA token&#41; input for optimal performance. AA-only sequence mode works but should be finetuned - frozen embeddings work only for SA input, not AA sequences! By contrast, SaProt-1.3B (SaProt_1.3B_AF2 & SaProt_1.3B_AFDB_OMG_NCBI) work for either SA token or AA-only token sequence. Despite that, with high-quality structural input, SaProt should surpasse ESM2 and  its own AA-only mode in most tasks.
